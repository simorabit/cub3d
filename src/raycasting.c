...
// /*   Updated: 2024/08/09 10:35:39 by mal-mora         ###   ########.fr       */
// /*                                                                            */
// /* ************************************************************************** */
...
//     {
//         if (window->map->v_map[(int)yintercept / TILE_SIZE][(int)xintercept / TILE_SIZE] == '1')
//         {
...
void DDA(int X1, int Y1, t_window *window, float scale_factor)
{
    int dx = X1 - (window->player.x + PLAYER_SIZE / 2);
    int dy = Y1 - (window->player.y + PLAYER_SIZE / 2);
    int i = 0;
    ...
    float X = (window->player.x + PLAYER_SIZE / 2) * scale_factor;
    float Y = (window->player.y + PLAYER_SIZE / 2) * scale_factor;

    while (i <= steps)
    {
        mlx_put_pixel(window->img, round(X) * SCALE_FACTOR, round(Y) * SCALE_FACTOR, 0xffff00ff);
        X += Xinc * scale_factor; // scale increment in x at each step
        Y += Yinc * scale_factor; // scale increment in y at each step
        i++;
    }
}
...
bool horizontal_casting(t_ray *ray, t_window *window)
{
    ...
    while (xintercept >= 0 && xintercept <= WIDTH && yintercept >= 0 && yintercept <= HEIGHT)
    {
        int map_x = xintercept / TILE_SIZE;
        int map_y = yintercept / TILE_SIZE;

        if (map_x >= 0 && map_x < window->map->width && map_y >= 0 && map_y < window->map->height &&
            window->map->v_map[map_y][map_x] == '1')
        {
            ray->wall_hit_x = xintercept;
            ray->wall_hit_y = yintercept;
            return true;
...
bool vertical_casting(t_ray *ray, t_window *window)
{
    float xintercept;
    float yintercept;
    float ystep;
...
void ray_casting(t_ray *ray, t_window *window, float scale_factor)
{
    bool findHorizontalwall;
    bool findVerticalWallHit;
    float horz_distance;
    ...
    init_ray(ray);
    findHorizontalwall = horizontal_casting(ray, window);
    findVerticalWallHit = vertical_casting(ray, window);
    horz_distance = (findHorizontalwall) ?
        distance_between_points(window->player.x, window->player.y, ray->wall_hit_x, ray->wall_hit_y) : INT_MAX;
    vert_distance = (findVerticalWallHit) ?
        distance_between_points(window->player.x, window->player.y, ray->wall_hit_x_ver, ray->wall_hit_y_ver) : INT_MAX;
    ...
    // Adjust the drawing to scale
    DDA(ray->wall_hit_x, ray->wall_hit_y, window, scale_factor);
}
...
void rays_casting(t_window *window)
{
    ...
    scale_factor = calculate_scale_factor(window);

    while (i < WIDTH)
    {
        window->ray_list[i].angle = normalize_angle(angle);
        ray_casting(&window->ray_list[i], window, scale_factor);
        i++;
        angle += FOV_ANGLE / WIDTH;
    }
}
