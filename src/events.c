...
/*   Updated: 2024/09/02 14:10:58 by mal-mora         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void update_player(t_window *window)
{
    ...
    if ((window->map->v_map[((int)round(new_y) + PLAYER_SIZE) / TILE_SIZE][((int)round(new_x) + PLAYER_SIZE) / TILE_SIZE] != '1' \
        && (window->map->v_map[((int)round(new_y)) / TILE_SIZE][((int)round(new_x) + PLAYER_SIZE) / TILE_SIZE] != '1') && \
        (window->map->v_map[((int)round(new_y) + PLAYER_SIZE) / TILE_SIZE][((int)round(new_x)) / TILE_SIZE] != '1')) && \
        (window->map->v_map[((int)round(new_y)) / TILE_SIZE][((int)round(new_x)) / TILE_SIZE] != '1'))
    {
        window->player.y = round(new_y);
        window->player.x = round(new_x);
    }
}
